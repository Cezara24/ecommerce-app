...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: 'Eroare la obținerea datelor de analiză.', details: error.message });
    }
  }
);
...
router.post(
  '/',
  authMiddleware,
  permissionMiddleware('create_analytics_event'),
  async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ error: 'Eroare la crearea evenimentului de analiză.', details: error.message });
    }
  }
);
...
router.get(
  ...
  async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ error: 'Eroare la obținerea evenimentelor de analiză.', details: error.message });
    }
  }
);
...
router.delete(
  '/:id',
  authMiddleware,
  permissionMiddleware('delete_analytics_event'),
  async (req, res) => {
    const { id } = req.params;
    ...
    try {
      ...
      res
        .status(500)
        .json({ error: 'Eroare la ștergerea evenimentului de analiză.', details: error.message });
    }
  }
);
